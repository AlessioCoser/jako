...
class ConditionTest {
    ...
    fun `eq as int`() {
        ...
        assertEquals("\"field\" = ?", condition.statement())
    ...
    fun `and`() {
        ...
        assertEquals("(\"field\" = ? AND \"field2\" = ?)", condition.statement())
    ...
    fun `or with different conditions`() {
        ...
        assertEquals("(\"field\" = ? OR \"field2\" > ?)", condition.statement())
        assertEquals(listOf("value", 2), condition.params())
    }
    ...
    fun `or and all together using dsl`() {
        ...
        assertEquals("((\"field\" = ? OR \"field2\" > ?) AND \"field3\" = ?)", condition.statement())
